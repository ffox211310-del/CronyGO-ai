...
// ★ パフォーマンス改善: 履歴制限
const MAX_HISTORY_MESSAGES = 6;  // AIに渡す会話数 (直近6往復で十分、トークン節約)
const MAX_ROOM_HISTORY = 40;      // 1ルームに保存する上限 (超えたら古い方から削除)
const MAX_DOM_MESSAGES = 30;      // 画面に描画する上限 (DOM重くなるのを防ぐ)
...
function addMessage(role, content) {
  ...
  if(allMsgs.length > MAX_DOM_MESSAGES + 5){
    // 最古の5個を削除 (WELCOMEは残す)
    let toRemove = allMsgs.length - MAX_DOM_MESSAGES;
    for(let i=0; i<allMsgs.length && toRemove>0; i++){
      if(allMsgs[i].id === 'loading-view') continue;
      if(allMsgs[i].textContent.includes('初回はモデルをダウンロード')) continue;
      allMsgs[i].remove();
      toRemove--;
    }
  }
...
function saveRoomMessages(id, msgs){
  try{
    // ★ 履歴上限: 古いものを削る
    let toSave = msgs;
    if(msgs.length > MAX_ROOM_HISTORY){
      toSave = msgs.slice(-MAX_ROOM_HISTORY);
      dbg(`[HistoryTrim] ${msgs.length} -> ${toSave.length} に削減`);
    }
    localStorage.setItem(LS_ROOM_PREFIX+id, JSON.stringify(toSave));
  ...
  roomListEl.innerHTML = rooms.map(r=>`
    <div class="room-item ${r.id===currentRoomId?'active':''}" data-id="${r.id}" style="display:flex; align-items:center; justify-content:space-between; gap:8px;">
      <div style="min-width:0; flex:1;">
        ...
        <span style="font-size:10px; opacity:0.5;">${new Date(parseInt(r.id)).toLocaleDateString()} (${loadRoomMessages(r.id).length}件)</span>
      </div>
...
function renderChatFromHistory(history){
  ...
  // ★ DOM最適化: 最新30件だけ描画、古いのはボタン
  let toRender = history;
  let omitted = 0;
  if(history.length > MAX_DOM_MESSAGES){
    omitted = history.length - MAX_DOM_MESSAGES;
    toRender = history.slice(-MAX_DOM_MESSAGES);
    const notice = document.createElement("div");
    notice.className = "msg system";
    notice.innerHTML = `過去${omitted}件は省略中 (パフォーマンスのため) <button id="load-all-history" style="margin-left:8px; background:#222; color:#4FD1C5; border:1px solid #333; border-radius:6px; padding:2px 8px; cursor:pointer;">全て表示</button>`;
    chatEl.appendChild(notice);
    notice.querySelector('#load-all-history')?.addEventListener('click', ()=>{
      clearChatUI();
      history.forEach(m=>{ addMessage(m.role, m.content); });
      chatEl.scrollTop = chatEl.scrollHeight;
    });
  }
  // DocumentFragmentで一括描画して高速化
...
function initRooms(){
  ...
  if(history.length > MAX_ROOM_HISTORY){
    trimmed = history.slice(-MAX_ROOM_HISTORY);
    saveRoomMessages(currentRoomId, trimmed);
  }
  messages = [{ role: "system", content: loadStoredPrompt() },...trimmed];
  renderChatFromHistory(trimmed);
  renderRoomList();
}
...
async function sendMessageWithText(forcedText) {
  ...
  if (isTimeQuery(text)) {
    ...
    // 履歴上限チェック
    if(messages.length > MAX_ROOM_HISTORY + 1){
      messages = [messages[0], ...messages.slice(-MAX_ROOM_HISTORY)];
    }
    saveCurrentRoomHistory();
    inputEl.value = "";
  ...
  try {
    ...
    // ★ ここが最重要: AIに渡すのは直近6件だけ
    const contextMessages = [
      messages[0],
      ...messages.slice(1).slice(-MAX_HISTORY_MESSAGES)
    ];
    dbg(`[Context] ${messages.length-1}件中 ${contextMessages.length-1}件をAIに渡す`);
