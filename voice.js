export class VoiceManager {
  ...
  _dbg(msg) {
    try { if (window.__cronyDbg) window.__cronyDbg(msg); } catch {}
  }
...
cleanForTTS(text) {
  ...
  init() {
    ...
    rec.onerror = (e) => {
      this._dbg(`[Voice] onerror ${e.error}`);
  ...
  async start() {
    ...
    this.wasListeningBeforeSpeak = false;
    clearTimeout(this._autoSendTimer);
    try { this.recognition.start(); this._dbg('[Voice] rec start ok'); } catch (e) {
      this._dbg('[Voice] rec start FAIL '+e.message);
  ...
  clearQueue(restartMic = true) {
    this._dbg(`[TTS] clearQueue restart=${restartMic}`);
    this._speakQueue = [];
    this._currentUtterance = null;
    try { window.speechSynthesis.cancel(); } catch {}
    this._isSpeakingQueue = false;
